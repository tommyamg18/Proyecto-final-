...
void agregar_jugador()
{
    ...
    else
    {
        ...
        jugadores[n_jugadores].nombre= leer_usuario();
        fprintf(s,"\t %s", jugadores[n_jugadores].nombre);
        fclose(s);
...
char *leer_usuario()
{
    char c, *usuario;
    int i=0;
    usuario= (char*)malloc(1 * sizeof(char));
    *usuario= '\0';
    while ((c=getchar())!='\n')
    {
        usuario= (char*)realloc(usuario, (i + 2)*sizeof(char));
        usuario[i++] = c;
        usuario[i] = '\0';
    }
    return usuario;
...
void juego_nombre()
{
    ...
    int i_jugador=0,i_jugador2=1;
    char help;

    do
    {
        imprimir_tablero();
        imprimir_jugador2(turno,i_jugador2,i_jugador2);
...
{
    if (turno == 0)
        printf("Juega %s: \n",jugadores[i_jugador].nombre);
    else if (turno == 1)
        printf("Juega %s: \n",jugadores[i_jugador2].nombre);
}
...
void cal_promedio()
{
    for(int i=0;i<n_jugadores;i++)
        {
            jugadores[i].promedio=(float)jugadores[i].partidas_ganadas/(jugadores[i].partidas_perdidas+jugadores[i].partidas_ganadas);
        }
}
...
int guardar_archivo_texto(player jugadores[100],int i)
{
    FILE * p,*p1,*p2,*p3;
    char *buff;
    buff=(char*)malloc(strlen(jugadores[i].nombre)*sizeof(char));

    p = fopen("nombre.txt", "w+");
    p1 = fopen("ganadas.txt", "w+");
    p2 = fopen("perdidas.txt", "w+");
    p3 = fopen("Promedio.txt", "w+");
    if (p==NULL){
        return 0;
    }
...
void inicializar_arreglo()
{
    for(int i=0;i<100;i++)
    {
         ...
         jugadores[i].partidas_perdidas=0.0;
